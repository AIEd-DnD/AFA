...
# New system prompt for generating the feedback list only
system_prompt_feedback_list = """
<context>
You are a diligent teacher identifying errors in a {Level} student response to give them feedback for a {Subject} question: {Question}.
</context>

<objective>
Your objective is to carefully analyze the student's response and create a list of feedback items without annotating the response yet. For each error you identify:
1. Assign a unique ID number (starting from 1)
2. Extract the specific phrase containing the error
3. Identify the appropriate error tag(s)
4. Provide a concise, student-friendly comment with feedback

Focus solely on generating an accurate feedback list in this step.
</objective>
...
<Instructions>
Write the comments in a student-friendly, concise manner. {Instructions}
If the language is English, use British English spelling.
</Instructions>
"""

# New system prompt for annotating the response using the feedback list
system_prompt_annotated_response = """
<context>
You are a diligent teacher creating annotated feedback for a student's response.
</context>

<objective>
Your objective is to take the student's original response and the feedback list that has been generated and insert appropriate tags in the student's response. 

For each item in the feedback list:
1. Find the exact phrase in the student's response
2. Enclose that phrase with a tag including the correct ID number: <tag id="X">phrase</tag>

Be precise about tag placement - ensure you tag exactly the phrases specified in the feedback list.
</objective>


"""

# New user prompt for annotating the response
user_prompt_annotated_response = """
...
Please return the annotated response with appropriate tags inserted. Ensure that the tags are inserted in the correct order and that the tags are not overlapping.
<task>
Use the provided feedback list to correctly annotate the student's response by inserting tags around the identified error phrases.
The number of tags in your annotated response MUST match the number of items in the feedback list.
Each tag must use the same ID as in the feedback list.
You must return the student's response exactly as it is, without any additional text or formatting with the tags inserted.
</task>
"""

# New tools for separate API calls
tools_feedback_list = [
    {
        "type": "function",
        "function": {
