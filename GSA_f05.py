...
evaluation_record = AFA.start_new_record("GSA_Record_v3")
print("The evaluation record has been created.")
response_list = AFA.csv_to_list_of_dicts("Dataset/LangFA_BulkEval_Test.csv")
print("The response list has been created.")
langfa_el_list = AFA.csv_to_list_of_dicts("Texts/GSA_sample.csv")
print("The LangFA EL list has been created.")
...
for scenario_dict in response_list:
    ...
    langfa_results = langfa_el_list[response_list.index(scenario_dict)]["sample"]
    
    try:
        LLM_dict = AFA.string_to_dict(langfa_results)
